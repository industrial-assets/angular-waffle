 ...
 * (c) 2015 Industrial Assets http://www.industrialassets.co.uk
 * License: MIT
 */
...
"", "", "", "", "", "", "", ""];
    ...
    /* the following are really just 'subject' phrases. In the sentence 'the cat
    sat on the mat', these phrases stand for 'the cat'. Normal bullsh*t does not
    even use a simple underlying sentence structure, but we are trying to keep
    things simple. */
    var $subjectPhrase = [
"|h strategic goals",
"|h gap analysis",
...
"a primary interrelationship between system and/or subsystem  technologies"];
    ...
    /* these are verb phrases ('sat', in 'the cat sat on the mat'). As we are
    imitating technological bullsh*t, we will use only the present tense for
    these.*/
...
"needs to be addessed along with the",
    ...
    /* These are object phrases. They are basically equivalent to the 'mat' in the
    classic simple sentence 'the cat sat on the mat'. */
    var $object_phrases = [
...
"the |c. This trend may dissipate due to the |c"];
    ...
    /* a nice collection of waffle adverbs that are used to make any pronouncement
    sound more impressive, much in the same way as garlic is used to make cooking
    taste french. */
    var $adverb = [
...
"retroactively", "retrospectively", "globally", "clearly", "disconcertingly"];
    ...
    /* a nice verb to floor the unwary */
...
"service", "support", "best-practice", "change", "delivery", "funding", "resources"];
    ...
    /* this bank of phrases are cliches that go well together. They were all
    found in real documents. we do this because, in real bullsh*t, people use
    cliches that sound peculiarly creative when jumbled up. */
    var $cliche = [
...
"corporate procedure", "proposed scenario"];
    ...
    /* one can sometimes puff up a buzzword by a preamble that substitutes for the
    'the' */
    var $prefix = [
...
"a preponderance of the"];
    ...
    var $ArtyNouns = ["discordance", "legitimisation", "principle", "transposition", "dimension",
...
"discord", "milieu"];
    ...
    var $surnames = ["Bennet", "Blotchet-Halls", "Carson", "Clarke", "DeFrance", "del Castillo", "Dull", "Green", "Greene",
...
"Straight", "Stringer", "White", "Yokomoto"];
    ...
    var $forenames = ["Abraham", "Reginald", "Cheryl", "Michel", "Innes", "Ann", "Marjorie", "Matthew", "Mark", "Luke", "John",
...
"Anne", "Meander", "Dean", "Dirk", "Desmond", "Akiko"];
    ...
    var $sequence = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven", "twelve"];

    var $ord_sequence = ["first", "second", "third", "fourth", "fifth"];
    ...
    function print_out(string, text_transform) {
        ...
        var preamblePhrase,
            ...
            verbPhrase,
            object_phrases,
            FirstAdjectivePhrase,
            SecondAdjectivePhrase,
            NounPhrase,
            prefix,
            adverb,
            cliche,
        ...
        if (string == '') return (''); /* if no string there */

        while (true) {

            if (count <= stringpointer) {
                return (buffered_string);
            };

            ch = string[stringpointer++];


            switch (ch) {
            case '|':
                /* the user wants something special */
                /* now lets look at what the user wants and act appropriately */
                ...
                case 'a':
                    buffered_string = buffered_string + print_out(cardinal_sequence());
                    break; /* print out  the next in the cardinal sequence (one two three)*/
                case 'b':
                    buffered_string = buffered_string + print_out(ordinal_sequence());
                    break; /* print out  the next in the ordinal  sequence (first, second, third)*/
                case 'c':
                    buffered_string = buffered_string + print_buzzword();
                    break; /* print out a random clichee */
                case 'd':
                    buffered_string = buffered_string + print_verb();
                    break; /* print out  a random verb */
                case 'e':
                    buffered_string = buffered_string + print_adverb();
                    break; /* print out  a random adverb */
                case 'f':
                    buffered_string = buffered_string + print_one_of($forenames);
                    break; /* print out  a forename at random */
                case 's':
                    buffered_string = buffered_string + print_one_of($surnames);
                    break; /* print out  a surname at random */
                case 'o':
                    buffered_string = buffered_string + print_one_of($ArtyNouns);
                    break; /* print out  an arty sort of noun */
                case 'y':
                    buffered_string = buffered_string + print_date();
                    break; /* print out  a  random date within the past thirty years */
                case 'h':
                    buffered_string = buffered_string + print_one_of($prefix);
                    break; /* print out  a 'the' phrase */
                case 'A':
                    buffered_string = buffered_string + print_one_of($preamblePhrase);
                    break; /* print out  a random preamble phase */
                case 'B':
                    buffered_string = buffered_string + print_one_of($subjectPhrase);
                    break; /* print out  a subject-phrase */
                case 'C':
                    buffered_string = buffered_string + print_one_of($verbPhrase);
                    break; /* print out  a verb-phrase */
                case 'D':
                    buffered_string = buffered_string + print_one_of($object_phrases);
                    break; /* print out  an object-phrase at random */
                case '1':
                    buffered_string = buffered_string + print_one_of($FirstAdjectivePhrase);
                    break; /* print out  a first part of a cliche at random */
                case '2':
                    buffered_string = buffered_string + print_one_of($SecondAdjectivePhrase);
                    break; /* print out  an adjective phrase at random */
                case '3':
                    buffered_string = buffered_string + print_one_of($NounPhrase);
                    break; /* print out  the third part of a cliche at random */
                case '4':
                    buffered_string = buffered_string + print_one_of($cliche);
                    break; /* print out  a cliche*/
                case 't':
                    buffered_string = buffered_string + titlesubject; //print_title();
                    break; /* print out  the title */
            ...
            default:
                column++;
                if ((sentence_start == true) && (ch.trim() != '')) {
                    ch = ch.charAt(0).toUpperCase() + ch.slice(1);
                    sentence_start = false;
                }
                buffered_string = buffered_string + ch; /* put out the character */
                if ((ch == '.') || (ch == ')')) sentence_start = true;
            }
        }
    }
    ...
    function print_buzzword() {
            return print_one_of($buzzphrases);
        }
        /*--------------------------------------------------------------------*/
    function print_one_of(string_array) {
            return print_out(string_array[Math.floor(Math.random() * string_array.length)]);
        }
        /*--------------------------------------------------------------------*/
    function print_date() {
        var date = new Date();
        var d2 = new Date(date.getFullYear() - Math.floor(Math.random() * 30), Math.floor(Math.random() * 12), Math.floor(Math.random() * 30));
        return d2.getMonth() + '/' + d2.getDay() + '/' + d2.getFullYear();
    }

    /*--------------------------------------------------------------------*/
    function print_verb() {
        return print_one_of($verb);
    }

    /*--------------------------------------------------------------------*/
    function print_adverb() {
        return print_one_of($adverb);
    }
    ...
    function print_title() {
            ...
            return capitalize_words(print_out("The |o of  |2 |o"));
        }
        /*--------------------------------------------------------------------*/
    function cardinal_sequence() {
        $so_far %= 6; /* force it between one and eleven */
        return $sequence[$so_far++];
    }

    /*--------------------------------------------------------------------*/
    function ordinal_sequence() {
        $done_so_far %= 12; /* force it between one and eleven*/
        if ($done_so_far < 6) return $ord_sequence[$done_so_far++];
        return $sequence[$done_so_far++] + 'th';
    }
    ...
    function $WaffleProvider() {

        var titlestart = print_out("The |o");
        titlesubject = print_out("|2 |o");
        title = capitalize_words(titlestart + ' of ' + titlesubject);

        this.$get = function () {

            return {
                generateParagraph: function (num) {
                    num = num | 0;
                    return $$generateWaffle(num, false);
                },
                generateTitle: function () {
                    return title; // print_title();
                },
                generateList: function (num) {
                    num = num | 0;
                    return $$generateWaffle(num, true);
                },
                generateQuote: function (num) {
                    num = num | 0;
                    var string = print_out("\"|A |B |C |t\"");
                    string += print_out(" - |f |s in") + capitalize_words(print_out(" The Journal of the |c (|y)"));
                    return string;
                },
                generateName: function (num) {
                    return print_one_of($forenames) + ' ' + print_one_of($surnames);
    ...
    function $$generateWaffle(times, do_word) {

        var string;

        if (do_word == true) { // Buzzwords

            string = [];

            for (var i = times; i > 0; i--) {
                var s = print_buzzword();
    ...
    // define ngWaffle module and register $waffle service
    angular.module('ngWaffle', []).provider('$waffle', $WaffleProvider);
